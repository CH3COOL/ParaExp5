...
//逆梯度改权 
{
	...
	//改Layer34 
	{
		...
		q->submit([&](handler& h) {
			...
			accessor delta4(buf_delta4, h, read_only);
			h.parallel_for(range<1>(batchNum * 10), [=](id<1>index) {
	...
	DeltaAft = DeltaBef;//2层的
	{
		...
		q->submit([&](handler&h) {
			...
			h.parallel_for(range<1>(batchNum*784), [=](id<1>index) {
				auto i = index % 784;
				auto b = index / 784;
				for (int j = 0; j < ImplN; j++)
					L12[i * ImplN + j] -= LEARN_RATE * delta2[b * ImplN + j] * batchdata[b * 784 + i];
			});
		});
		q->submit([&](handler&h) {
